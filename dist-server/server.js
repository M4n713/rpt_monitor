    ...
    if (dbInitStatus.mode === 'mock') {
        ...
        if (lowerText.startsWith('insert into')) {
            ...
            if (table && mockStore[table]) {
                const newId = mockStore[table].length + 1;
        ...
        if (lowerText.startsWith('update')) {
            ...
            if (table && mockStore[table]) {
                ...
                if (idMatch) {
                    ...
                    const items = mockStore[table].filter(i => i.id === id);
                    items.forEach(item => {
        ...
        if (lowerText.startsWith('delete')) {
            ...
            if (table && mockStore[table]) {
                ...
                if (idMatch) {
                    const idParamIndex = parseInt(idMatch[1]) - 1;
                    const id = params[idParamIndex];
                    const initialLength = mockStore[table].length;
                    mockStore[table] = mockStore[table].filter(i => i.id !== id);
                    return { rowCount: initialLength - mockStore[table].length };
                }
            }
        }
...
const canAccessProperty = async (user, propertyId) => {
    ...
    if (user.role === 'taxpayer') {
        const { rows } = await dbQuery('SELECT 1 FROM property_owners WHERE property_id = $1 AND user_id = $2 LIMIT 1', [propertyId, user.id]);
    ...
    if (user.role === 'collector') {
       ...
       WHERE po.property_id = $1 AND u.assigned_collector_id = $2
       LIMIT 1`, [propertyId, user.id]);
        return rows.length > 0;
    }
    return false;
};
const getAuthorizedPropertiesByPins = async (user, pins) => {
    ...
    if (user.role === 'collector') {
        const { rows } = await dbQuery(`SELECT DISTINCT p.*
       FROM properties p
       JOIN property_owners po ON po.property_id = p.id
       JOIN users u ON u.id = po.user_id
       WHERE p.pin = ANY($1) AND u.assigned_collector_id = $2`, [pins, user.id]);
        return rows;
    }
    if (user.role === 'taxpayer') {
        const { rows } = await dbQuery(`SELECT DISTINCT p.*
       FROM properties p
       JOIN property_owners po ON po.property_id = p.id
       WHERE p.pin = ANY($1) AND po.user_id = $2`, [pins, user.id]);
        return rows;
    }
    return [];
};
...
const initDb = async (retries = 5, delay = 2000) => {
    ...
    for (let i = 0; i < retries; i++) {
      ...
      `).catch(e => console.log('Alter table custom_computation_types failed:', e.message));
        ...
        END $$;
      `);
            await dbQuery('ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_pin_key').catch(() => { });
...
async function startServer() {
    ...
    app.post('/api/admin/reset-password', authenticateToken, async (req, res) => {
        try {
            ...
            if (!['admin', 'collector', 'taxpayer'].includes(role)) {
            ...
            await dbQuery('INSERT INTO admin_logs (action_type, details, admin_id, created_at) VALUES ($1, $2, $3, $4)', ['user_management', `Reset password for user ID ${userId}`, req.user.id, new Date().toISOString()]);
    ...
    app.post('/api/register', async (req, res) => {
        ...
        try {
            ...
            const { rows } = await dbQuery('INSERT INTO users (username, password, role, full_name) VALUES ($1, $2, $3, $4) RETURNING id', [normalizedUsername, hashedPassword, 'taxpayer', normalizedFullName]);
    ...
    app.post('/api/admin/create-taxpayer', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            const temporaryPassword = generateTemporaryPassword();
            const hashedPassword = bcrypt.hashSync(temporaryPassword, 10);
            const { rows } = await dbQuery('INSERT INTO users (username, password, role, full_name) VALUES ($1, $2, $3, $4) RETURNING id', [normalizedUsername, hashedPassword, 'taxpayer', full_name]);
            await dbQuery('INSERT INTO admin_logs (action_type, details, admin_id, created_at) VALUES ($1, $2, $3, $4)', ['user_management', `Created taxpayer: ${full_name} (${normalizedUsername})`, req.user.id, new Date().toISOString()]);
    ...
    app.post('/api/admin/create-admin', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden: Admin access required' });
        if (req.user.username.toLowerCase() !== 'manlie')
            return res.status(403).json({ error: 'Forbidden: Only Manlie can create admins' });
        const { username, full_name } = req.body;
        if (!username || !full_name)
            return res.status(400).json({ error: 'Missing required fields' });
        try {
            ...
            const temporaryPassword = generateTemporaryPassword();
            const hashedPassword = bcrypt.hashSync(temporaryPassword, 10);
            const { rows } = await dbQuery('INSERT INTO users (username, password, role, full_name) VALUES ($1, $2, $3, $4) RETURNING id', [normalizedUsername, hashedPassword, 'admin', full_name]);
            await dbQuery('INSERT INTO admin_logs (action_type, details, admin_id, created_at) VALUES ($1, $2, $3, $4)', ['user_management', `Created admin: ${full_name} (${normalizedUsername})`, req.user.id, new Date().toISOString()]);
            res.json({ success: true, id: rows[0].id, username: normalizedUsername, full_name, temporaryPassword });
        }
        catch (error) {
            if (error.code === '23505')
    ...
    app.post('/api/admin/create-collector', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden: Admin access required' });
        if (req.user.username.toLowerCase() !== 'manlie')
            return res.status(403).json({ error: 'Forbidden: Only Manlie can create collectors' });
        const { username, full_name } = req.body;
        if (!username || !full_name)
            return res.status(400).json({ error: 'Missing required fields' });
        try {
            ...
            const temporaryPassword = generateTemporaryPassword();
            const hashedPassword = bcrypt.hashSync(temporaryPassword, 10);
            const { rows } = await dbQuery('INSERT INTO users (username, password, role, full_name) VALUES ($1, $2, $3, $4) RETURNING id', [normalizedUsername, hashedPassword, 'collector', full_name]);
            await dbQuery('INSERT INTO admin_logs (action_type, details, admin_id, created_at) VALUES ($1, $2, $3, $4)', ['user_management', `Created collector: ${full_name} (${normalizedUsername})`, req.user.id, new Date().toISOString()]);
            res.json({ success: true, id: rows[0].id, username: normalizedUsername, full_name, temporaryPassword });
        }
        catch (error) {
            if (error.code === '23505')
    ...
    app.get('/api/properties', authenticateToken, async (req, res) => {
        ...
        `, [p.id]);
                ...
                const ownerNames = owners.map((o) => o.full_name).join(', ');
    ...
    app.post('/api/properties/by-pins', authenticateToken, async (req, res) => {
        try {
            const { pins } = req.body;
            ...
            const sanitizedPins = pins
                .map((pin) => typeof pin === 'string' ? pin.trim() : '')
                .filter(Boolean);
    ...
    app.get('/api/collector/payments', authenticateToken, async (req, res) => {
        if (req.user.role !== 'collector' && req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
            const { rows } = await dbQuery(`
    ...
    app.post('/api/admin/link-property', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden: Admin access required' });
        ...
        if (!propsToLink && Array.isArray(property_ids)) {
            propsToLink = property_ids.map((id) => ({ id, ownership_type, claimed_area }));
        }
        ...
        try {
            ...
            for (const propData of propsToLink) {
                ...
                if (!existingLink) {
                    await dbQuery('INSERT INTO property_owners (property_id, user_id, ownership_type, claimed_area) VALUES ($1, $2, $3, $4)', [id, taxpayer_id, ownership_type, claimed_area]);
                    // Update legacy fields
                    await dbQuery('UPDATE properties SET owner_id = $1, ownership_type = $2, claimed_area = $3 WHERE id = $4', [taxpayer_id, ownership_type, claimed_area, id]);
                    // Get PIN for logging
            ...
            if (linkedPins.length > 0) {
                // Check for open log
                const { rows: openLogs } = await dbQuery('SELECT id, pins FROM taxpayer_logs WHERE taxpayer_id = $1 AND time_out IS NULL ORDER BY created_at DESC LIMIT 1', [taxpayer_id]);
    ...
    app.post('/api/admin/unlink-property', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden: Admin access required' });
    ...
    app.post('/api/admin/upload-abstract', authenticateToken, upload.single('file'), async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden: Admin access required' });
        if (!req.file)
            ...
            try {
                ...
                for (const row of results) {
                    try {
                        ...
                        if (!prop)
                            throw new Error(`Property with PIN ${pin} not found`);
              ...
              `, [prop.id, amount, date, req.user.id, or_no, year, basic, sef, interest, discount]);
                ...
                await dbQuery('INSERT INTO admin_logs (action_type, details, admin_id, created_at) VALUES ($1, $2, $3, $4)', ['system_data', `Uploaded historical RPT Abstract: ${successCount} records imported`, req.user.id, new Date().toISOString()]);
                await dbQuery('COMMIT');
    ...
    app.post('/api/admin/upload-roll', authenticateToken, upload.single('file'), async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden: Admin access required' });
        if (!req.file)
            return res.status(400).json({ error: 'No file uploaded' });
        ...
        stream
            ...
            `);
                    for (const row of stagedRows) {
                        await client.query(`INSERT INTO roll_import_stage (
              ...
              ORDER BY row_number
            `);
                    await client.query('INSERT INTO admin_logs (action_type, details, admin_id, created_at) VALUES ($1, $2, $3, $4)', ['system_data', `Uploaded new Tax Roll via staged import: ${stagedRows.length} properties imported`, req.user.id, new Date().toISOString()]);
    ...
    app.post('/api/collector/view-taxpayer', authenticateToken, async (req, res) => {
        if (req.user.role !== 'collector' && req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        const { taxpayer_id } = req.body;
        if (!taxpayer_id)
            return res.status(400).json({ error: 'Taxpayer ID required' });
        try {
            const { rows: tpRows } = await dbQuery('SELECT full_name FROM users WHERE id = $1', [taxpayer_id]);
            if (tpRows.length === 0)
                return res.status(404).json({ error: 'Taxpayer not found' });
            ...
            // Check for open log
            const { rows: openLogs } = await dbQuery('SELECT id FROM taxpayer_logs WHERE taxpayer_id = $1 AND user_id = $2 AND time_out IS NULL ORDER BY created_at DESC LIMIT 1', [taxpayer_id, req.user.id]);
            if (openLogs.length === 0) {
                // Create new log with time_in
                await dbQuery(`
          INSERT INTO taxpayer_logs (taxpayer_id, taxpayer_name, role, user_id, user_name, pins, time_in, time_out, created_at)
          VALUES ($1, $2, $3, $4, $5, $6, $7, NULL, $8)
        `, [taxpayer_id, taxpayerName, req.user.role, req.user.id, req.user.name, '[]', new Date().toISOString(), new Date().toISOString()]);
    ...
    app.get('/api/admin/taxpayers', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
    ...
    app.post('/api/queue/register', async (req, res) => {
        try {
            ...
            let taxpayer;
            const { rows: existingRows } = await dbQuery('SELECT id, username, full_name, role FROM users WHERE (phone_number = $1 OR full_name = $2) AND role = $3 LIMIT 1', [phone_number, full_name, 'taxpayer']);
            ...
            if (existingRows.length > 0) {
                ...
                await dbQuery('UPDATE users SET queue_number = $1, queue_date = $2, notified = FALSE, notified_at = NULL WHERE id = $3', [nextNum, currentDay, taxpayer.id]);
            }
            else {
                ...
                const { rows: insertRows } = await dbQuery('INSERT INTO users (username, password, role, full_name, age, gender, phone_number, queue_number, queue_date) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id, username, full_name, queue_number, phone_number', [username, hashedPassword, 'taxpayer', full_name, age, gender, phone_number, nextNum, currentDay]);
    ...
    app.post('/api/admin/notify-taxpayer', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        const { taxpayer_id } = req.body;
        try {
            ...
            const tp = rows[0];
            if (tp.notified)
                return res.json({ message: 'Already notified', success: true });
            if (!tp.phone_number)
    ...
    app.post('/api/admin/cancel-queue', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        const { taxpayer_id } = req.body;
        try {
            await dbQuery('UPDATE users SET queue_number = NULL, queue_date = NULL, notified = FALSE, notified_at = NULL WHERE id = $1', [taxpayer_id]);
    ...
    app.get('/api/admin/collectors', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
            const { rows } = await dbQuery(`
    ...
    app.post('/api/admin/clear-assignment', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        const { taxpayer_id } = req.body;
        try {
    ...
    app.get('/api/collector/taxpayers', authenticateToken, async (req, res) => {
        if (req.user.role !== 'collector' && req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
            let query;
    ...
    app.post('/api/payment', authenticateToken, async (req, res) => {
      ...
      `, paymentParams);
            ...
            if (taxpayer_id) {
                ...
                const { rows: openLogs } = await dbQuery('SELECT id, pins FROM taxpayer_logs WHERE taxpayer_id = $1 AND user_id = $2 AND time_out IS NULL ORDER BY created_at DESC LIMIT 1', [taxpayer_id, req.user.id]);
                if (openLogs.length > 0) {
    ...
    app.get('/api/admin/debug-users', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
    ...
    app.post('/api/admin/temp-delete-users', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
    ...
    app.get('/api/taxpayer-logs', authenticateToken, async (req, res) => {
    ...
    app.post('/api/taxpayer-logs', authenticateToken, async (req, res) => {
        if (req.user.role !== 'collector' && req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
    ...
    app.post('/api/admin/barangays', authenticateToken, async (req, res) => {
        if (req.user.username.toLowerCase() !== 'manlie')
            return res.status(403).json({ error: 'Forbidden' });
    ...
    app.delete('/api/admin/barangays/:id', authenticateToken, async (req, res) => {
        if (req.user.username.toLowerCase() !== 'manlie')
            return res.status(403).json({ error: 'Forbidden' });
        try {
    ...
    app.get('/api/admin/computation-types', authenticateToken, async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(403).json({ error: 'Forbidden' });
        try {
        ...
        try {
            ...
            payments.forEach((p) => {
                ...
                if (p.year) {
                    ...
                    if (p.year.includes('-')) {
                        const [start, end] = p.year.split('-').map((y) => parseInt(y.trim()));
            ...
            for (const prop of properties) {
                ...
                for (let y = startYear; y <= currentYear; y++) {
                    ...
                    if (y === currentYear && !isApril1Passed)
                        continue;
    ...
    app.post('/api/direct-messages/:id/read', authenticateToken, async (req, res) => {
        try {
    ...
    app.post('/api/inquiries', async (req, res) => {
        ...
        try {
            await dbQuery('INSERT INTO inquiries (sender_name, email, message) VALUES ($1, $2, $3)', [sender_name, email, message]);
            res.json({ success: true });
        }
        catch (err) {
