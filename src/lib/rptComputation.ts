export type BaseComputationType = 'standard' | 'rpvara' | 'denr' | 'share';
export type SpecialCaseHook = 'none' | 'rpvara_2024_half' | 'denr_10_year_window';
...
export interface ComputationRuleConfig {
  advance_discount_rate: number;
  share_advance_discount_rate: number;
  current_year_discount_rate: number;
  current_year_interest_rate: number;
  delinquent_interest_rate: number;
  delinquent_cap_post_1991: number;
  delinquent_cap_pre_1992: number;
  skip_years_older_than: number | null;
  waive_delinquent_interest: boolean;
  waive_delinquent_discount: boolean;
}
...
export interface ComputationRule {
  ...
  label: string;
  value: string;
  base_type: BaseComputationType;
  special_case_hook: SpecialCaseHook;
  description?: string | null;
  effective_from?: string | null;
  effective_to?: string | null;
  is_active: boolean;
  is_builtin?: boolean;
  config: Partial<ComputationRuleConfig>;
}

type ComputationProperty = {
  assessed_value?: number | string | null;
  total_area?: number | string | null;
  claimed_area?: number | string | null;
};

const DEFAULT_RULE_CONFIG: ComputationRuleConfig = {
...
export const DEFAULT_COMPUTATION_RULES: ComputationRule[] = [
  {
    label: 'Standard',
    value: 'standard',
    base_type: 'standard',
...
export const getPHTimeNow = (): Date => {
  const utcDate = new Date();
  return new Date(utcDate.getTime() + (8 * 60 * 60 * 1000));
};

export const normalizeRule = (rule: ComputationRule): ComputationRule => ({
  ...rule,
...
export const getAvailableRules = (rules?: ComputationRule[]) => {
...
export const resolveComputationRule = (selectedValue: string, rules?: ComputationRule[]) => {
...
export const isRuleEffective = (rule: ComputationRule, now: Date = getPHTimeNow()) => {
...
const parseAssessedValue = (manualAssessedValue?: number | string | null, prop?: ComputationProperty) => {
  const rawAssessed = manualAssessedValue ?? prop?.assessed_value ?? 0;
  if (typeof rawAssessed === 'number') return isNaN(rawAssessed) ? 0 : rawAssessed;

  const parsed = parseFloat(String(rawAssessed).replace(/[^0-9.-]/g, ''));
  return isNaN(parsed) ? 0 : parsed;
};

const getShareAdjustedValue = (assessedValue: number, prop?: ComputationProperty) => {
...
export const calculateTaxForYear = (
  year: number,
  prop: ComputationProperty,
  selectedRuleValue: string,
  manualAssessedValue?: number | string,
  options: CalculationOptions = {},
  rules?: ComputationRule[],
  now: Date = getPHTimeNow()
) => {
  const currentYear = now.getFullYear();
  const currentMonth = now.getMonth();
  const rule = normalizeRule(resolveComputationRule(selectedRuleValue, rules));
  const config = rule.config as ComputationRuleConfig;
  ...
  if (year > currentYear) {
    const advanceDiscountRate =
      rule.base_type === 'share'
...
export const calculateTaxForRange = (
